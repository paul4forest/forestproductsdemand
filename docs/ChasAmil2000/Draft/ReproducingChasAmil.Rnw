...
\title{Reproducing estimates of Chas-Amil and Buongiorno "The demand for paper and paperboard - Econometric model for the European Union"}
\author{Paul Rougieux}
\begin{document}
\maketitle

\begin{abstract}
Chas Amil and buongiorno (2000) estimated demand functions for paper and paperboard products in the European Union (EU15). In the first section, we attempt to reproduce their graphs and estimates for the period 1996-1992, based on FAOSTAT forest products production and trade data and World Bank GDP and deflator data. In a second section we extended the estimation period by 20 years, until 2012. In a third section, we estimated for all EU27 countries but only over the period from 1992 until 2012. 
\end{abstract}
...
\newpage
\section{Reproducing Chas Amil and Buongiorno}
\subsection{Consumption of Paper and Paperboard (1969-1995)}
\begin{figure}[h]
\centering
...
# Rename total volume and change Tons to Thousand Tons
ppc$Item[ppc$Item=="Paper and Paperboard"] = "Total"
...
\newpage
\subsection{Price of paper and paperboard (1969-1995)}
\begin{figure}[h]
\centering
...
\newpage
\subsection{GDP and consumption per capita}
...
pp95 = merge(subset(pp, Year==1995 & Item=="Paper and Paperboard", 
...
# Remove 1969 because Germany doesn't have GDP for that year
tp = subset(pp, Year>=1970&Year<=1992&Item=="Paper and Paperboard"&
                # Remove Ireland
                Country!="Ireland")

# subset(tp,log(GDPconstantUSD)==-Inf) # To find issues in the regressors
...
dynmodel = function(df){
  ...
  coefs$Country = unique(df$Country)
  if (nrow(coefs)==2){
...
x = ddply(tp, .(Country), dynmodel)
x = x[c( "Country", "log.GDPconstantUSD.", "log.Price.", 
         "log.Consum_t_1.", "R_Squared")] 
x[,-1] = round(x[,-1],2)
x$Country[is.na(x$R_Squared)] = ""

print(xtable(x),  type = "latex",)


# Transform into a matrix 
# # Because sanitation requires special column names which I could not use in a data.frame
# mat = data.matrix(x[-1])
# rownames(mat) = x$Country
# colnames(mat) = c( "GDP", "P","$C_{t-1}$", "${R}^2$")
...
\subsection{Consumption of Paper and Paperboard (1969-2012)}
...
\subsection{Price of paper and paperboard (1969-2012)}
