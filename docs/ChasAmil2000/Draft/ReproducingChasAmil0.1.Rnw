...
\subsection{Chart of Paper and Paperboard consumption (1969-1995)}
\begin{figure}[h]
\centering
...
\newpage
\subsection{Chart of paper and paperboard price (1969-1995)}
\begin{figure}[h]
\centering
...
tp = subset(pp, Year>=1969&Year<=1992&Item=="Paper and Paperboard"&
                # Remove Ireland
                Country!="Ireland")
...
# Change product to an ordered vector, same as Buongiorno
y$Product[y$Product=="Paper and Paperboard"] = "Total Paper and Paperboard"
y$Product = factor(y$Product, 
                   levels=c("Total Paper and Paperboard", "Newsprint",
                            "Printing+Writing Paper", "Other Paper+Paperboard"), 
                   ordered=TRUE)
y = y[order(y$Product),]
